        ...
        if editor is not None:
            ...
            try:
                ...
                if parser.getNumberOfSyntaxErrors() > 0:
                    return self.error_callback(parser.getNumberOfSyntaxErrors())
                ...
                try:
                    ...
                    print(result)
                    with open('ci_to_cm/outputs/mips_code.asm', 'r') as f:
                        mips_code = f.read()

                    self.mips_text.setText(mips_code)
                    if not (self.mips_frame in self.hsplit.children()):
                        self.hsplit.replaceWidget(0, self.mips_frame)
                    # self.hsplit.replaceWidget(0, self.mips_frame)
                    
                except Exception as e:
                    print(e)
                    self.terminal_text.append("> Compilation to MIPS failed")
                    if not (self.terminal_frame in self.hsplit.children()):
                        self.hsplit.replaceWidget(0, self.terminal_frame)
                    # self.terminal_text.setText("> Compilation to MIPS failed")
            except:
                #
                pass
...
#--------------------------------------------------
    ...
    def set_up_body(self):
        ...
        mips_label = self.get_side_bar_label('gui/icons/MIPS.png', 'MIPS')
        side_bar_layout.addWidget(mips_label)
        ...
        # MIPS (assembly) view
        self.mips_frame = self.get_frame()
        self.mips_frame.setMaximumWidth(400)
        self.mips_frame.setMinimumWidth(200)

        mips_frame_layout = QVBoxLayout()
        mips_frame_layout.setAlignment(Qt.AlignmentFlag.AlignTop)
        mips_frame_layout.setContentsMargins(0, 10, 0, 0)
        mips_frame_layout.setSpacing(0)

        self.mips_text = QTextEdit()
        self.mips_text.setReadOnly(True)
        self.mips_text.setFont(self.window_font)
        self.mips_text.setAlignment(Qt.AlignmentFlag.AlignTop)
        self.mips_text.setStyleSheet('''
                                        background-color: #21252b;
                                        border-radius: 5px;
                                        border: none;
                                        padding: 5px;
                                        color: #D3D3D3;
                                        ''')
        mips_frame_layout.addWidget(self.mips_text)
        self.mips_frame.setLayout(mips_frame_layout)
